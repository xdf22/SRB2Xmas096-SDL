...
typedef enum
{
    ...
    MF_JUMPED            = 32,
    // Stealth: Is Spinning on the ground
    MF_SPINNING          = 64,
    //SOM: If it hit the floor, flag this for one tick!
    MF_JUSTJUMPED        = 128,
    //Stealth: Reving Spindash
    MF_STARTDASH         = 256,
    // Tails Flying Tails 03-03-2000
    MF_TAILSFLY          = 512,
} mobjeflag_t;
...
typedef struct mobj_s
{
    ...
    // faB: BIG HACK: for release, z of water in current sector
    // used by P_XYMovement(), set at start of mobjthinker
    fixed_t             waterz;

    // WARNING : nexw field are not automaticely added to save game
    //SOM: Added fuse
    int                 fuse;
