...
//
void P_FindAnimatedFlat (animdef_t*   animdef)
{
    int            i;
    int            startflatnum,endflatnum;
    levelflat_t*   foundflats = levelflats;

    startflatnum = W_CheckNumForName (animdef->startname);
    if (startflatnum == -1)
        return;  // skip anims not in Doom1 ?

    endflatnum = W_GetNumForName (animdef->endname);
...
void P_UpdateSpecials (void)
{
    ...
    for (anim = anims ; anim < lastanim ; anim++)
    {
        ...
        {
            //if (anim->istexture)
            texturetranslation[i] = anim->basepic +
